...
GLubyte white[3] = { 255, 255, 200 }; //makes screen yellow
...
void draw(const sound_key_definition & source, const sound_key_instance & data)
{
	...
	const GLfloat zoom_size = 40.0f;
	const GLfloat prompt_size = 15.9f;
...
void load(const sound_key_definition & source, sound_key_instance & data)
{
	...
	glBindTexture(GL_TEXTURE_2D, data.prompt);
	load_bitmap(source.prompt);
