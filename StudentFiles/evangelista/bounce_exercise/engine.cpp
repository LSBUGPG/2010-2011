...
static bool extension_is_supported(const char * extension)
{
     PFNWGLGETEXTENSIONSSTRINGEXTPROC list_of_extensions = NULL;
     list_of_extensions = (PFNWGLGETEXTENSIONSSTRINGEXTPROC) wglGetProcAddress("wglGetExtensionsStringEXT");
     return (strstr(list_of_extensions(), extension) != NULL);
}

static void set_swap_control(void)
{
     // note: the space at the end of the extension name is significant
     if (extension_is_supported("WGL_EXT_swap_control ")) {
          PROC extension = wglGetProcAddress("wglSwapIntervalEXT");
          if (extension != NULL) {
               PFNWGLSWAPINTERVALEXTPROC set_swap_interval = (PFNWGLSWAPINTERVALEXTPROC)extension;
               if (!set_swap_interval(1)) {
                    _cputs("Could not set swap interval!\n");
               }
          } else {
               _cputs("Could not find the swap interval function!\n");
          }
     } else {
          _cputs("Swap interval extension not supported!\n");
     }
}
...
GLuint load_bitmap(const char * filename)
{
     ...
     if (surface == NULL) {
          ...
          _cprintf(
               ...
               "This engine only draws 24 bit bitmaps.\n",
