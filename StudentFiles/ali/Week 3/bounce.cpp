...
void update(const ball_type & type, ball_instance & instance, float time)
{
	...
	if (fabs(instance.position[0]) > 320.0f) {
		instance.velocity[0] *= -1.0f; 
		play_audio(type.audio);
	} 

	instance.velocity[1] -= 3000.0f * time; // acceleration due to gravity in mm
}
...
int main(int argc, char * argv[])
{
	...
	ball_definition definition = { "shuriken.bmp", "metal clang.wav", 40.0f };
	ball_type type;
