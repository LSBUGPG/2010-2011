...
class ball_instance: public object {                // class of ball_instance, based on public version of "object"
	GLfloat position[2];
...
protected:
	...
	{
		for (int index = 0; index < number_of_balls; index++) {     //goes through each ball
			function.call(ball[index]);								// calls ball function on each ball
		}
	}                                                     
};
