...
void update(const ball_type & type, ball_instance & instance, float time)
{
	...
	instance.position[1] += instance.velocity[1] * time;
	instance.position[0] += instance.velocity[5] * time;
	}

	instance.velocity[1] -= 10.0f * time; // acceleration due to gravity in mm
}
...
int main(int argc, char * argv[])
{
	...
	ball_definition definition = { "ball.bmp", "bounce.wav", 60.0f };
	ball_type type;
