...
struct ball_instance {
     GLfloat position[1];//this makes the motion restrained by "friction"
     GLfloat velocity[1];
};
...
const GLfloat base = GLfloat(-screen_height / 4); //move the bounce upwards a bit, previous value /2
...
void update(const ball_type & type, ball_instance & instance, float time)
{
     ...
     if (distance < 0.0f) {
		  ...
		  if (instance.position[0] < -270)
		  {
		 instance.velocity[0]= 0.01;	 
		  }
	 }

     instance.velocity[1] -= 2000.0f * time; // acceleration due to gravity in mm
	 instance.velocity[0] -= 100.0f * time; // a == less gravity 
}
